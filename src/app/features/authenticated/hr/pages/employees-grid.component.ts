...
import { EmployeesHttpService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
  ...
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3 class="text-2xl">RH / Employées</h3>
          </div>
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="employeeFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        <my-label>Nom *</my-label>
                        <input #firstFocused formControlName="name" type="text" myInput >
                        <my-error *ngIf="employeeFilterFormGroup.get('name')?.invalid && (employeeFilterFormGroup.get('name')?.dirty || employeeFilterFormGroup.get('name')?.touched) && employeeFilterFormGroup.get('name')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Employée</span>
              </div>
            </button>
            </div>
          </div>
  ...
  styles: [`
      app-employees-grid { display: flex; flex: 1; }
    `],
})
export class EmployeesGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'name', 'salary', 'debt', 'phoneNumberOne', 'phoneNumberTow', 'actions'];
  resultsLength = 0;
  ...
  employeeFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.employeeFilterFormGroup = this.fb.group({
      'name': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.employeeFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.employeesHttp
  ...
  deleteItem(item: any): void {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette employée?")) {
      this.deleteMany([item]);
    }
  }
  deleteSeleted(): void {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette employées?")) {
      this.deleteMany(this.selection.selected);
    }
  }

  filterItems(): void {
    this.employeeFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.employeeFilterFormGroup.reset({
      'name': '',
    });
    this.employeeFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.employeesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.employeeFilterFormGroup.get('name')?.value,
    }
