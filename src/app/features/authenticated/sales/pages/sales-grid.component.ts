...
import { MoneySourcesHttpService, SalesHttpService, CustomersHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
            ...
            <div class="flex flex-row items-center gap-x-3">
              ...
              <button mat-flat-button color="primary" (click)="newItem()">
                <div class="!flex !flex-row !items-center !space-x-2">
                  ...
                  <span class="text-white">Vente</span>
                </div>
              </button>
            </div>
          </div>
            ...
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="customerId.name">
                  ...
                  <td mat-cell *matCellDef="let row">{{ row.customerId.name }}</td>
                </ng-container>
                ...
                <ng-container matColumnDef="deliveryAmount">
                  <th mat-header-cell *matHeaderCellDef>Montant de livraison</th>
                  <td mat-cell *matCellDef="let row">{{ row.deliveryAmount }}</td>
                </ng-container>
  ...
  styles: [`
      app-sales-grid { display: flex; flex: 1; }
    `],
})
export class SalesGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'customerId.name', 'moneySourceId.label', 'deliveryAmount', 'payment', 'totalQuantity', 'totalAmount', 'date', 'actions'];
  resultsLength = 0;
  isLoadingResults = true;
  isRateLimitReached = false;
  isFilterMenuOpen: boolean = false;
  saleFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  saleFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.saleFilterFormGroup = this.fb.group({
      'customerId': [undefined],
      'moneySourceId': [undefined],
    });
  }
  ngOnInit() {
    this.customersHttp.getAll().subscribe({
      next: customers => {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.saleFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.salesHttp
  ...
  filterItems(): void {
    this.saleFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.saleFilterFormGroup.reset({
      'customerId': undefined,
      'moneySourceId': undefined,
    });
    this.saleFilterChanged.emit();
  ...
  deleteMany(items: any[]) {
    ...
    this.salesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      customerId: this.saleFilterFormGroup.get('customerId')?.value,
      moneySourceId: this.saleFilterFormGroup.get('moneySourceId')?.value,
    }
    ...
    if (isNotEmpty(qry.customerId)) urlParams.append('customerId', qry.customerId);
