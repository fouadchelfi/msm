...
import { LosseNaturesHttpService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
import { MatDialog } from '@angular/material/dialog';
import { LosseNatureFormComponent } from './losse-nature-form.component';
import { SelectionModel } from '@angular/cdk/collections';
import { appConfig } from '../../../../app.config';

@Component({
  selector: 'app-losse-natures-grid',
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3 class="text-2xl">Trésoreries / Natures pertes</h3>
          </div>
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="losseNatureFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        <my-label>Libellé *</my-label>
                        <input #firstFocused formControlName="label" type="text" myInput >
                        <my-error *ngIf="losseNatureFilterFormGroup.get('label')?.invalid && (losseNatureFilterFormGroup.get('label')?.dirty || losseNatureFilterFormGroup.get('label')?.touched) && losseNatureFilterFormGroup.get('label')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Nature perte</span>
              </div>
            </button>
            </div>
          </div>
  ...
  styles: [`
      app-losse-natures-grid { display: flex; flex: 1; }
    `],
})
export class LosseNaturesGridComponent implements OnInit {
  ...
  losseNatureFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  losseNatureFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.losseNatureFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.losseNatureFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.LosseNaturesHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(LosseNatureFormComponent, {
  ...
  filterItems(): void {
    this.losseNatureFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.losseNatureFilterFormGroup.reset({
      'label': '',
    });
    this.losseNatureFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.LosseNaturesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.losseNatureFilterFormGroup.get('label')?.value,
    }
