...
@Component({
          ...
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="ingredientFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        <my-label>Libellé *</my-label>
                        <input #firstFocused formControlName="label" type="text" myInput >
                        <my-error *ngIf="ingredientFilterFormGroup.get('label')?.invalid && (ingredientFilterFormGroup.get('label')?.dirty || ingredientFilterFormGroup.get('label')?.touched) && ingredientFilterFormGroup.get('label')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Ingrédient</span>
              </div>
            </button>
            </div>
          </div>
  ...
  styles: [`
      app-ingredients-grid { display: flex; flex: 1; }
    `],
})
export class IngredientsGridComponent implements OnInit {
  ...
  ingredientFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  ingredientFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.ingredientFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.ingredientFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.ingredientsHttp
  ...
  filterItems(): void {
    this.ingredientFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.ingredientFilterFormGroup.reset({
      'label': '',
    });
    this.ingredientFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.ingredientsHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.ingredientFilterFormGroup.get('label')?.value,
    }
