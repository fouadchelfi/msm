...
import { PremisesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
                  ...
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="premiseFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                      <my-form-field>
                        ...
                        <my-error *ngIf="premiseFilterFormGroup.get('label')?.invalid && (premiseFilterFormGroup.get('label')?.dirty || premiseFilterFormGroup.get('label')?.touched) && premiseFilterFormGroup.get('label')?.getError('required')">
  ...
  styles: [`
      app-premises-grid { display: flex; flex: 1; }
    `],
})
export class PremisesGridComponent implements OnInit {
  ...
  premiseFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  premiseFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.premiseFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.premiseFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.premisesHttp
  ...
  filterItems(): void {
    this.premiseFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.premiseFilterFormGroup.reset({
      'label': '',
    });
    this.premiseFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.premisesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.premiseFilterFormGroup.get('label')?.value,
    }
