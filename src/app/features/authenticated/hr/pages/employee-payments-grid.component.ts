...
import { EmployeesHttpService, EmployeePaymentsHttpService, TraceabilityService, isEmpty, isNotEmpty, MoneySourcesHttpService } from '../../../../shared';
...
@Component({
  selector: 'app-employee-payments-grid',
  template: `
    ...
    <div>
      <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
        <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt" matSortDisableClear
          ...
          <ng-container matColumnDef="calculatedPayment">
            <th mat-header-cell *matHeaderCellDef>Paiement calculé</th>
            <td mat-cell *matCellDef="let row">{{ row.calculatedPayment }}</td>
          </ng-container> 

          <ng-container matColumnDef="restPayment">
            <th mat-header-cell *matHeaderCellDef>Restant</th>
            <td mat-cell *matCellDef="let row">{{ row.restPayment }}</td>
          </ng-container> 
  ...
  styles: [`
      app-employee-payments-grid { display: flex; flex: 1; }
    `],
})
export class EmployeePaymentsGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'employeeId.name', 'calculatedPayment', 'payment', 'restPayment', 'moneySourceId.label', 'date', 'actions'];
  resultsLength = 0;
  ...
  deleteMany(items: any[]) {
    ...
    this.paymentsHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
