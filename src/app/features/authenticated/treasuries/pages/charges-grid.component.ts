...
import { ChargesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
          ...
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="chargeFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        <my-label>Nom *</my-label>
                        <input #firstFocused formControlName="label" type="text" myInput >
                        <my-error *ngIf="chargeFilterFormGroup.get('label')?.invalid && (chargeFilterFormGroup.get('label')?.dirty || chargeFilterFormGroup.get('label')?.touched) && chargeFilterFormGroup.get('label')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Charge</span>
              </div>
            </button>
            </div>
          </div>
          <div>
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="chargeNatureId.label">
                  <th mat-header-cell *matHeaderCellDef >Nature </th>
                  <td mat-cell *matCellDef="let row">{{ row.chargeNatureId.label }}</td>
                </ng-container>
  ...
  styles: [`
      app-charges-grid { display: flex; flex: 1; }
    `],
})
export class ChargesGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'label', 'chargeNatureId.label', 'amount', 'date', 'actions'];
  resultsLength = 0;
  isLoadingResults = true;
  isRateLimitReached = false;
  isFilterMenuOpen: boolean = false;
  chargeFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  chargeFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.chargeFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.chargeFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.chargesHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(ChargeFormComponent, {
  ...
  filterItems(): void {
    this.chargeFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.chargeFilterFormGroup.reset({
      'label': '',
    });
    this.chargeFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.chargesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.chargeFilterFormGroup.get('label')?.value,
    }
