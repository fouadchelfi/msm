...
import { CustomersHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
  ...
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3 class="text-2xl">Ventes / Clients</h3>
          </div>
                  ...
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="customerFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        ...
                        <my-error *ngIf="customerFilterFormGroup.get('name')?.invalid && (customerFilterFormGroup.get('name')?.dirty || customerFilterFormGroup.get('name')?.touched) && customerFilterFormGroup.get('name')?.getError('required')">
  ...
  styles: [`
      app-customers-grid { display: flex; flex: 1; }
    `],
})
export class CustomersGridComponent implements OnInit {
  ...
  customerFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  customerFilterChanged = new EventEmitter();
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.customerFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.customersHttp
  ...
  filterItems(): void {
    this.customerFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.customersHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.customerFilterFormGroup.get('name')?.value,
    }
