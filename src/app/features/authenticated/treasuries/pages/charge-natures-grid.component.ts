...
import { ChargeNaturesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
import { MatDialog } from '@angular/material/dialog';
import { ChargeNatureFormComponent } from './charge-nature-form.component';
import { SelectionModel } from '@angular/cdk/collections';
import { appConfig } from '../../../../app.config';

@Component({
  selector: 'app-charge-natures-grid',
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3 class="text-2xl">Stock / Natures charges</h3>
          </div>
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="chargeNatureFilterFormGroup" class="flex flex-col !text-sm gap-y-4 p-6">
                      <my-form-field>
                        <my-label>Libellé *</my-label>
                        <input #firstFocused formControlName="label" type="text" myInput >
                        <my-error *ngIf="chargeNatureFilterFormGroup.get('label')?.invalid && (chargeNatureFilterFormGroup.get('label')?.dirty || chargeNatureFilterFormGroup.get('label')?.touched) && chargeNatureFilterFormGroup.get('label')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Nature charge</span>
              </div>
            </button>
            </div>
          </div>
  ...
  styles: [`
      app-charge-natures-grid { display: flex; flex: 1; }
    `],
})
export class ChargeNaturesGridComponent implements OnInit {
  ...
  chargeNatureFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  chargeNatureFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.chargeNatureFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.chargeNatureFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.ChargeNaturesHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(ChargeNatureFormComponent, {
  ...
  deleteItem(item: any): void {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette nature charge?")) {
      this.deleteMany([item]);
    }
  }
  deleteSeleted(): void {
    if (confirm("Êtes-vous sûr de vouloir supprimer cette nature charges?")) {
      this.deleteMany(this.selection.selected);
    }
  }

  filterItems(): void {
    this.chargeNatureFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.chargeNatureFilterFormGroup.reset({
      'label': '',
    });
    this.chargeNatureFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.ChargeNaturesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.chargeNatureFilterFormGroup.get('label')?.value,
    }
