...
import { FamiliesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
          ...
          <div class="flex flex-row justify-between items-center ">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  class="flex flex-col min-w-[400px] max-h-[80vh] overflow-auto bg-white shadow-lg border border-gray-200 rounded-sm">
                    ...
                    <form [formGroup]="familyFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                      <my-form-field>
                        <my-label>Libellé *</my-label>
                        <input #firstFocused formControlName="label" type="text" myInput >
                        <my-error *ngIf="familyFilterFormGroup.get('label')?.invalid && (familyFilterFormGroup.get('label')?.dirty || familyFilterFormGroup.get('label')?.touched) && familyFilterFormGroup.get('label')?.getError('required')">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Famille</span>
              </div>
            </button>
            </div>
          </div>
  ...
  styles: [`
      app-families-grid { display: flex; flex: 1; }
    `],
})
export class FamiliesGridComponent implements OnInit {
  ...
  familyFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  familyFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.familyFilterFormGroup = this.fb.group({
      'label': [''],
    });
  }
  ngOnInit() { }
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.familyFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.familiesHttp
  ...
  filterItems(): void {
    this.familyFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.familyFilterFormGroup.reset({
      'label': '',
    });
    this.familyFilterChanged.emit();
  }
  ...
  deleteMany(items: any[]) {
    ...
    this.familiesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      label: this.familyFilterFormGroup.get('label')?.value,
    }
