...
import { QuantityCorrectionsHttpService, StocksHttpService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
import { MatDialog } from '@angular/material/dialog';
import { SelectionModel } from '@angular/cdk/collections';
import { appConfig } from '../../../../app.config';
import { QuantityCorrectionFormComponent } from './quantity-correction-form.component';

@Component({
  selector: 'app-quantity-corrections-grid',
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3>Stocks / Corrections quantité</h3>
          </div>
          <div class="flex flex-row justify-between items-center ">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Correction</span>
              </div>
            </button>
            </div>
          </div>
          <div>
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="stockId.label">
                  <th mat-header-cell *matHeaderCellDef >Libellé </th>
                  <td mat-cell *matCellDef="let row">{{ row.stockId.label|myLimitTextLength:30 }}</td>
                </ng-container>
      
                <ng-container matColumnDef="oldQuantity">
                  <th mat-header-cell *matHeaderCellDef >Ancienne quantité </th>
                  <td mat-cell *matCellDef="let row">{{ row.oldQuantity }}</td>
                </ng-container>
      
                <ng-container matColumnDef="newQuantity">
                  <th mat-header-cell *matHeaderCellDef >Nouvelle quantité </th>
                  <td mat-cell *matCellDef="let row">{{ row.newQuantity }}</td>
                </ng-container>
  ...
  styles: [`
      app-quantity-corrections-grid { display: flex; flex: 1; }
    `],
})
export class QuantityCorrectionsGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'stockId.label', 'oldQuantity', 'newQuantity', 'date', 'actions'];
  resultsLength = 0;
  ...
  constructor(
    ...
    private quantityCorrectionsHttp: QuantityCorrectionsHttpService,
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.stockFilterChanged)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.quantityCorrectionsHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(QuantityCorrectionFormComponent, {
  ...
  deleteMany(items: any[]) {
    ...
    this.quantityCorrectionsHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      stockId: this.stockFilterFormGroup.get('stockId')?.value,
    }
    ...
    if (isNotEmpty(qry.stockId)) urlParams.append('stockId', qry.stockId);
