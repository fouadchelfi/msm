...
import { EmployeesHttpService, PunchesHttpService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
import { MatDialog } from '@angular/material/dialog';
import { SelectionModel } from '@angular/cdk/collections';
import { appConfig } from '../../../../app.config';
import { PuncheFormComponent } from './punche-form.component';

@Component({
  selector: 'app-punches-grid',
  template: `
  ...
  styles: [`
      app-punches-grid { display: flex; flex: 1; }
    `],
})
export class PunchesGridComponent implements OnInit {
  ...
  deleteMany(items: any[]) {
    ...
    this.punchesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
