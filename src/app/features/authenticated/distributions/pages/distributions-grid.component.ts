...
import { MoneySourcesHttpService, DistributionsHttpService, PremisesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
  selector: 'app-distributions-grid',
  template: `
            ...
            <div class="relative">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  ...
                  <form [formGroup]="distributionFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                    <my-form-field>
            ...
            <div class="flex flex-row items-center gap-x-3">
              ...
              <button mat-flat-button color="primary" (click)="newItem()">
                <div class="!flex !flex-row !items-center !space-x-2">
                  ...
                  <span class="text-white">Distribution</span>
                </div>
              </button>
            </div>
          </div>
            ...
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="cash">
                  <th mat-header-cell *matHeaderCellDef>Montant en espèces</th>
  ...
  styles: [`
      app-distributions-grid { display: flex; flex: 1; }
    `],
})
export class DistributionsGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'premiseId.label', 'moneySourceId.label', 'cash', 'totalQuantity', 'totalAmount', 'date', 'actions'];
  resultsLength = 0;
  isLoadingResults = true;
  isRateLimitReached = false;
  isFilterMenuOpen: boolean = false;
  distributionFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  distributionFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.distributionFilterFormGroup = this.fb.group({
      'premiseId': [undefined],
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.distributionFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.distributionsHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(DistributionFormComponent, {
  ...
  filterItems(): void {
    this.distributionFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.distributionFilterFormGroup.reset({
      'premiseId': undefined,
      'moneySourceId': undefined,
    });
    this.distributionFilterChanged.emit();
  ...
  deleteMany(items: any[]) {
    ...
    this.distributionsHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      premiseId: this.distributionFilterFormGroup.get('premiseId')?.value,
      moneySourceId: this.distributionFilterFormGroup.get('moneySourceId')?.value,
    }
