...
import { FenceFormComponent } from './fence-form.component';

@Component({
  selector: 'app-fences-grid',
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3>Trésoreries / Clôtures</h3>
          </div>
          <div class="flex flex-row justify-between items-center">
            ...
            <button mat-flat-button color="primary" (click)="newItem()">
              <div class="!flex !flex-row !items-center !space-x-2">
                ...
                <span class="text-white">Clôture</span>
              </div>
            </button>
            </div>
          </div>
            ...
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] !w-[calc(100vw-140px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="totalPurchaseAmount">
                  <th mat-header-cell *matHeaderCellDef >Montant d'achat</th>
                  <td mat-cell *matCellDef="let row">
  ...
  styles: [`
      app-fences-grid { display: flex; flex: 1; }
    `],
})
export class FencesGridComponent implements OnInit {
  ...
  deleteMany(items: any[]) {
    ...
    this.fencesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      categoryId: this.transferFilterFormGroup.get('categoryId')?.value,
    }
