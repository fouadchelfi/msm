...
@Component({
  ...
  template: `
        <div class="flex flex-1 flex-col bg-white gap-y-2">
          <div class="p-3">
              ...
              <div class="relative">
                ...
                <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                  ...
                  <div
                    ...
                    <form [formGroup]="userFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                      <my-form-field>
                        <my-label>Nom *</my-label>
                        <input #firstFocused formControlName="name" type="text" myInput>
                        <my-error
                          *ngIf="userFilterFormGroup.get('name')?.invalid && (userFilterFormGroup.get('name')?.dirty || userFilterFormGroup.get('name')?.touched) && userFilterFormGroup.get('name')?.getError('required')">
                          Veuillez remplir ce champ.
                        </my-error>
                      </my-form-field>
                    </form>
                    <div class="flex flex-row justify-between p-6">
                      <button (click)="resetItemsFilterForm()" mat-stroked-button
                        color="secondary">Réinitialiser</button>
          ...
          <div>
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-300px)] bg-white">
  ...
  styles: [`
      app-users-grid { display: flex; flex: 1; }
    `],
})
export class UsersGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'name', 'notes', 'actions'];
  resultsLength = 0;
  ...
  userFilterChanged = new EventEmitter();
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.userFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
  ...
  deleteMany(items: any[]) {
    ...
    this.usersHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      name: this.userFilterFormGroup.get('name')?.value,
    }
