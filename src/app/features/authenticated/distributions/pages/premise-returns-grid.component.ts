...
import { MoneySourcesHttpService, PremiseReturnsHttpService, PremisesHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
import { MatDialog } from '@angular/material/dialog';
import { PremiseReturnFormComponent } from './premise-return-form.component';
import { SelectionModel } from '@angular/cdk/collections';
import { appConfig } from '../../../../app.config';

@Component({
  selector: 'app-premise-returns-grid',
  template: `
        <div class="flex flex-1 flex-col p-3 bg-secondary-50 gap-y-2">
          <div class="flex flex-col">
            <h3>Distributions / Retours</h3>
          </div>
            ...
            <div class="relative">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  ...
                  <form [formGroup]="premiseReturnFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                    <my-form-field>
          ...
          <div>
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="premiseId.label">
                  <th mat-header-cell *matHeaderCellDef>Local </th>
                  <td mat-cell *matCellDef="let row">{{ row.premiseId.label }}</td>
                </ng-container>
                ...
                <ng-container matColumnDef="returnedCash">
                  <th mat-header-cell *matHeaderCellDef>Espèces retournées</th>
                  <td mat-cell *matCellDef="let row">{{ row.returnedCash }}</td>
                </ng-container>
  ...
  styles: [`
      app-premise-returns-grid { display: flex; flex: 1; }
    `],
})
export class PremiseReturnsGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'premiseId.label', 'moneySourceId.label', 'returnedCash', 'totalQuantity', 'totalAmount', 'date', 'actions'];
  resultsLength = 0;
  isLoadingResults = true;
  isRateLimitReached = false;
  isFilterMenuOpen: boolean = false;
  premiseReturnFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  premiseReturnFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.premiseReturnFilterFormGroup = this.fb.group({
      'premiseId': [undefined],
      'moneySourceId': [undefined],
    });
  }
  ngOnInit() {
    this.premisesHttp.getAll().subscribe({
      next: premises => {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.premiseReturnFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.premiseReturnsHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(PremiseReturnFormComponent, {
  ...
  filterItems(): void {
    this.premiseReturnFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.premiseReturnFilterFormGroup.reset({
      'premiseId': undefined,
      'moneySourceId': undefined,
    });
    this.premiseReturnFilterChanged.emit();
  ...
  deleteMany(items: any[]) {
    ...
    this.premiseReturnsHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      premiseId: this.premiseReturnFilterFormGroup.get('premiseId')?.value,
      moneySourceId: this.premiseReturnFilterFormGroup.get('moneySourceId')?.value,
    }
    ...
    if (isNotEmpty(qry.premiseId)) urlParams.append('premiseId', qry.premiseId);
