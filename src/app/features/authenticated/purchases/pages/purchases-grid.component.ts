...
import { MoneySourcesHttpService, PurchasesHttpService, SuppliersHttpService, TimeZoneService, TraceabilityService, isEmpty, isNotEmpty } from '../../../../shared';
...
@Component({
            ...
            <div class="relative">
              ...
              <ng-template cdkConnectedOverlay [cdkConnectedOverlayOrigin]="trigger"
                ...
                <div
                  ...
                  <form [formGroup]="purchaseFilterFormGroup" class="flex flex-col !text-sm gap-y-2 p-5">
                    <my-form-field>
                      <my-label>Fournisseur</my-label>
            ...
            <div class="flex flex-row items-center gap-x-3">
              ...
              <button mat-flat-button color="primary" (click)="newItem()">
                <div class="!flex !flex-row !items-center !space-x-2">
                  ...
                  <span class="text-white">Achat</span>
                </div>
              </button>
            </div>
          </div>
            ...
            <div class="relative flex-1 overflow-auto !h-[calc(100vh-185px)] bg-white rounded shadow">
              <table mat-table [dataSource]="dataSource" class="w-full" matSort matSortActive="createdAt"
                ...
                <ng-container matColumnDef="payment">
                  <th mat-header-cell *matHeaderCellDef>Versement</th>
  ...
  styles: [`
      app-purchases-grid { display: flex; flex: 1; }
    `],
})
export class PurchasesGridComponent implements OnInit {

  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = ['select', 'code', 'supplierId.name', 'moneySourceId.label', 'cost', 'payment', 'totalQuantity', 'totalAmount', 'date', 'actions'];
  resultsLength = 0;
  isLoadingResults = true;
  isRateLimitReached = false;
  isFilterMenuOpen: boolean = false;
  purchaseFilterFormGroup: FormGroup;
  refreshGrid = new EventEmitter();
  purchaseFilterChanged = new EventEmitter();
  ...
  constructor(
    ...
    this.purchaseFilterFormGroup = this.fb.group({
      'supplierId': [undefined],
      'moneySourceId': [undefined],
    });
  }
  ngOnInit() {
    this.suppliersHttp.getAll().subscribe({
      next: suppliers => {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.refreshGrid, this.purchaseFilterChanged)
      .pipe(
        startWith({}),
        switchMap(() => {
          this.isLoadingResults = true;
          return this.purchasesHttp
  ...
  newItem(action: 'creation' | 'edit' = 'creation', id: number = 0): void {
    this.matDialog.open(PurchaseFormComponent, {
  ...
  filterItems(): void {
    this.purchaseFilterChanged.emit();
  }

  resetItemsFilterForm(): void {
    this.purchaseFilterFormGroup.reset({
      'supplierId': undefined,
      'moneySourceId': undefined,
    });
    this.purchaseFilterChanged.emit();
  ...
  deleteMany(items: any[]) {
    ...
    this.purchasesHttp.deleteMany(query).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  get getFilterQuery(): string {
    ...
    const qry: any = {
      ...
      supplierId: this.purchaseFilterFormGroup.get('supplierId')?.value,
      moneySourceId: this.purchaseFilterFormGroup.get('moneySourceId')?.value,
    }
    ...
    if (isNotEmpty(qry.supplierId)) urlParams.append('supplierId', qry.supplierId);
